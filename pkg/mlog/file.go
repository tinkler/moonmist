...
type fileLogger interface {
	FileTime() time.Time
	NewFile()
	Log(message []byte, level messageLevel)
	Close() error
}
...
func (fl *oneFileLogger) Log(message []byte, level messageLevel) {
...
func (fl separateFileLogger) Log(message []byte, level messageLevel) {
	switch level {
	case ML_ERR:
...
func writeToFile(message []byte, level messageLevel) {
...
func (lf *logFile) NewFile(path string) error {
	...
	if err != nil {
		writeRuntimeMessage()
		os.Stdout.Write([]byte(wrapMessage(Format(err), ML_ERR)))
		return err
	}
	lf.file = f
	return nil
